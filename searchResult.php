<?php
    session_start();
    require 'Class_Folder/searchClass.php';
...
<html>
    ...
    <div class="wrapper">
        ...
        <div class="clearfix">
            ...
            <!--- - - - - - - - -  SEARCHBAR ENDS - - - - - - - - --->
            <?php
            if(!isset($_POST['keySearch']) || empty($_POST['keySearch']))
                {
                
                echo "<br />"."You did not insert any keyword(s), please try agian";
            }
            else {
                $keywords = $_POST['keySearch'];
                $result = new searchClass();
                list($resultNum, $recipeResults, $mediaResults) = $result -> searchByKeywords($keywords)
            ?>
            <br />
            <h3><?php echo $resultNum." " ?> result(s) for <?php echo $keywords ?></h3>
            <br/>
            <table>
                <?php foreach ($recipeResults as $value): ?>
                <tr>
                    <!-- by default hidden_cat is search because in the refer page we have switch statement with the name of the search with display search result  -->
                    <td><a href="recipeLink.php?idRecipe=<?php echo $value['idRecipe'] ?>"><?php echo $value['recipe_name'] ?></a></td>
                </tr>
                <tr>
                    <td class="searchContent"><?php echo $value['instruction'] ?></td>
                </tr>
                <tr><td><br/></td></tr>
                <?php endforeach; ?>
                <?php foreach ($mediaResults as $value): ?>
                <tr>
                    <td class="searchtitle"><a href="foodnetwork.php?hidden_cat=search&idMedia=<?php echo $value['idMedia'] ?>"><?php echo $value['title'] ?></a></td>
                </tr>
                <tr>
                    <td class="searchContent"><?php echo $value['content'] ?></td>
                </tr>
                <tr><td><br/></td></tr>
                <?php endforeach; ?>
            </table>          
